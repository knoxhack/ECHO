...
Build-truth validation token: `1.2.0`.
Compatibility history token: `1.2.0`.
...
The current full-stack release set is `echocore` `1.2.0`, `echonetcore` `1.2.0`, `echoruntimeguard` `1.2.0`, `echothemecore` `1.2.0`, `echoplayercore` `1.2.0`, `echomissioncore` `1.2.0`, `echodatacore` `1.2.0`, `echoworldcore` `1.2.0`, `echoterminal` `1.2.0`, `signalos` `1.2.0`, `signalosexample` `1.2.0`, `echorendercore` `1.2.0`, `echoashfallprotocol` `1.2.0`, `echoorbitalremnants` `1.2.0`, `echonexusprotocol` `1.2.0`, `echoagriculturereclamation` `1.2.0`, `echostationfall` `1.2.0`, `echoblackboxprotocol` `1.2.0`, `echoindustrialnexus` `1.2.0`, `echologisticsnetwork` `1.2.0`, `echoconvoyprotocol` `1.2.0`, `echoholomap` `1.2.0`, `echoindex` `1.2.0`, `echoarmory` `1.2.0`, `echolens` `1.2.0`, `echomultiblockcore` `1.2.0`, and `echoblockworks` `1.2.0`.
...
| Module | Version | Role |
|---|---:|---|
| `echocore` | `1.2.0` | Shared service layer for pack mode, profile state, progress ledgers, diagnostics, hazards, route records, factions, POI affinity, terminal reward storage, archives, intel mirrors, terminal placement, and Nexus campaign state. |
| `echonetcore` | `1.2.0` | Shared packet bridge, sync helpers, packet diagnostics, and server action validation. |
...
| `echomissioncore` | `1.2.0` | Shared mission service, objective registry, progression state, and reward contracts. |
| `echodatacore` | `1.2.0` | Shared persistent player, world, and team data service. |
| `echoworldcore` | `1.2.0` | Shared world regions, markers, hazards, structure discoveries, and world event contracts. |
| `echoterminal` | `1.2.0` | Shared ECHO terminal shell, mission browser, chapter navigation, archive surfaces, reward inbox, diagnostics, and addon-facing navigation/profile API. |
| `signalos` | `1.2.0` | Reusable terminal/content framework for chapters, missions, archives, rewards, diagnostics, JSON loading, validation, and soft KubeJS bridge integration. |
| `signalosexample` | `1.2.0` | Example-only SignalOS addon demonstrating Java registration, datapack JSON, diagnostics, rewards, archives, and KubeJS-friendly content. |
| `echorendercore` | `1.2.0` | Shared visual-state, animation-profile, particle-profile, preview, composition, and renderer helper layer for advanced ECHO/Ashfall assets. |
| `echoashfallprotocol` | `1.2.0` | Main ruined-Earth survival campaign with the compact drop pod, hazards, machines, factions, guardians, Prime Relay warfront, Nexus choice, Warden finale, and addon handoff. |
| `echoorbitalremnants` | `1.2.0` | Post-Nexus orbital route chain from Earth calibration through launch, Low Orbit, Station Network, Moon, Mars, Europa, Saturn, Titan, Deep Space Protocol, ECHO-0, surveys, faction contracts, and final network seal. |
| `echoagriculturereclamation` | `1.2.0` | Field agriculture recovery with recovered seeds, contaminated soils, hydroponics, greenhouse zones, Pollinator Drone service, gene stabilization, bio-reactor support, and chunk-local restoration scores. |
| `echostationfall` | `1.2.0` | Station ECHO horror chapter with station boarding, nine section power/log recovery, oxygen/pressure/panic telemetry, AI override, Station Mother, and Blackbox handoff. |
| `echonexusprotocol` | `1.2.0` | Chapter IV Nexus corruption and memory chapter with Nexus Charge, smarter field-map risk planning, field stabilization, corrupted biomes, Core access, matter rewriting, the Nexus Guardian, and Restore/Control/Destroy/Merge path commitment. |
| `echoindustrialnexus` | `1.2.0` | Industrial automation chapter with Thermal Flux, ducts, MultiblockCore factories, Factory Command dashboards, machine heat, scrubber safe zones, filter automation, hybrid Nexus processing, POIs, and the Furnace Warden. |
| `echologisticsnetwork` | `1.2.0` | Supply crates, labels, loadouts, external endpoints, drone delivery docks, remote requests, faction depots, courier persistence, Terminal integration, and operations dashboards. |
| `echoconvoyprotocol` | `1.2.0` | Ruined-Earth vehicles, multiblock depots, cargo/fuel logistics, deterministic Field Ops, HoloMap routes, roadside contracts, recovery signals, and travel hazards. |
...
| `echoarmory` | `1.2.0` | Modular weapons, armor, workstations, modules, energy recharge, faction locks, Terminal hooks, and Logistics loadout hooks. |
...
| `echoblockworks` | `1.2.0` | First-party decorative, structural, and themed block families for ECHO builds and ruins. |
| `echoblackboxprotocol` | `1.2.0` | Late-game memory finale with typed Blackbox fragments, archive dungeons, memory stability, hostile recordings, boss proofs, Nexus Core Access Key, Truth Engine, and final directives. |
...
The Gradle workspace supports a beta addon set and the full public stack. `-PechoAddonSet=beta` includes NetCore, RuntimeGuard, ThemeCore, PlayerCore, Terminal, MissionCore, DataCore, SignalOS, SignalOS Example, RenderCore, HoloMap, Lens, Orbital Remnants, Nexus Protocol, Agriculture Reclamation, WorldCore, MultiblockCore, Blockworks, PowerGrid, SoundCore, TutorialCore, RelicTech, and WeatherCore, plus Core and ECHO: Ashfall Protocol. `-PechoAddonSet=all` adds Armory, Blackbox Protocol, Convoy Protocol, Index, Industrial Nexus, Logistics Network, and Stationfall.

Release verification is also part of the ecosystem. `build -PechoAddonSet=all` is the required full-stack build command for this workspace pass. `verifyEchoRelease` runs resource validation, gameplay-data validation, POI checks, runtime-log checks, builds, and GameTests without requiring a local launcher profile. `copyEchoJarsToModpack`, `checkEchoModJarSet`, and `verifyEchoModpackProfile` are explicit local modpack-profile checks and require `-PechoModpackModsDir` for the intended Ashfall profile. The private `tools/echo-release-terminal` dashboard supports release drafting and QA state, but it is not shipped as a mod artifact.
...
Build truth includes `echopowergrid` `1.2.0`, `echosoundcore` `1.2.0`, `echotutorialcore` `1.2.0`, `echorelictech` `1.2.0`, and `echoweathercore` `1.2.0` in the explicit public stack. These modules are active but still have partial integration/audio/tutorial/weather surfaces documented in the ecosystem audit reports.
