...
```powershell
cd "C:\New folder\Echo\addons\echomodpackcommandcenter"
npm.cmd install
npm.cmd run dev
npm.cmd test
npm.cmd run build
...
- Quick Scan inspects JSON validity, resource references, lang coverage signals, terminal page signals, handoff docs, runtime log signatures, and the configured modpack jar set.
- Deep Scan adds optional Python validator adapters for resource, gameplay, structure, and runtime-log checks.
- Settings persist the ECHO root, modpack mods folder, Python executable, runtime log age, and default scan mode in the local SQLite database.
