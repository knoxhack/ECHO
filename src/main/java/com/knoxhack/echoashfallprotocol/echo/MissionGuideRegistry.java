...
public final class MissionGuideRegistry {
    private static final Guide FALLBACK = new Guide(
            "ECHO FIELD NOTE",
            "ECHO has no field guide for this protocol yet.");
    ...
    private static final Map<String, Guide> GUIDES = Map.ofEntries(
            ...
            Map.entry("drink_clean_water", guide("Clean Water", "Clean Water is the safe first drink. Confirm a starter bottle before scouting, then make dirty water collection and purification your next loop.")),
            Map.entry("get_dirty_water", guide("Dirty Water", "Dirty Water is emergency-usable when a route goes bad, but purification is safer. Fill bottles from water sources and save them for a Water Purifier.")),
            Map.entry("emergency_filter_water", guide("Emergency Clean Water", "Dirty Water + Basic Filter + Ash makes one Clean Water Bottle by hand. It consumes the full filter, so use it to survive until the Water Purifier is online.")),
            Map.entry("craft_scrap_knife", guide("Scrap Knife", "The Scrap Knife is your first salvage blade. Craft it after the pod outpost is anchored, then claim the scrap reward before extending water access.")),
            Map.entry("secure_crash_outpost", guide("Ash Campfire", "The Ash Campfire is your first outpost anchor. Craft it from 2 Sticks and 2 Ash, place it near the pod, then add reward storage and light nearby.")),
            Map.entry("forage_wasteland_food", guide("Food Buffer", "Wild berries and emergency rations buy time while machines come online. Starter rations are a valid safety buffer; berries are extra route margin.")),
            Map.entry("plant_mutated_sapling", guide("Mutated Sapling", "Mutated saplings give renewable wood in a ruined biome. Claim this mission reward because it includes the cauldron and scrap plastic for the Rain Collector.")),
            Map.entry("build_rain_collector", guide("Rain Collector", "Rain Collectors turn storms into dirty water. Craft it from the rewarded cauldron and 7 Scrap Plastic, then purify bottles before long routes.")),
            ...
            Map.entry("secure_sleep_shelter", guide("Sleep Shelter", "A bed and lit shelter give you a retreat point. Claim this reward before crafting the bone tools and hide wrap; those rewards bridge into the schematic step.")),
            ...
            Map.entry("find_schematic_fragment", guide("Schematic Fragment", "Schematic Fragments restore lost crafting knowledge. This early chain gives one through ECHO so the first Hand Recycler is not random-loot gated.")),
            Map.entry("build_hand_recycler", guide("Hand Recycler", "The Hand Recycler is the first machine bridge. It now uses the guaranteed schematic casing, 4 Scrap Metal, and 4 Scrap Wire; power it with the starter battery or a nearby generator.")),
            ...
            Map.entry("base_stability_check", guide("Base Stability", "A stable outpost has storage, water purification, buffered power, and a small recovery kit. Confirm this before toxic-zone gear and longer routes.")),
            ...
            Map.entry("contact_remnants", guide("Remnant Job Sites", "Right-click a Supply Crate or Weapon Rack at a Remnant outpost. Remnants offer security tasks, quartermaster support, and military-grade progression.")),
            Map.entry("contact_salvagers", guide("Salvager Job Sites", "Right-click a Trade Counter or Map Table at a Salvager post. Salvagers offer trade runs, salvage work, maps, and route information.")),
            Map.entry("contact_mutants", guide("Mutant Job Sites", "Right-click a Bio Processing Station or Spore Garden at a Mutant enclave. Mutants offer medicine, adaptation research, and biological recovery routes.")),
            ...
            Map.entry("craft_portable_scanner", guide("Portable Signal Scanner", "The Portable Signal Scanner searches for nearby exploration sites and reports route, hazard, prep, supplies, and objective state.")),
            ...
            Map.entry("scan_first_poi", guide("First POI Scan", "Run the portable scanner until ECHO records a structure. Each site now has a route, hazard profile, useful gear hint, reward track, and field-log state.")),
            ...
            Map.entry("recover_data_log", guide("Data Logs", "Data Logs explain why a location matters and often point toward systems or dangers you will meet later.")),
            ...
            Map.entry("repair_echo_drone", guide("Drone Recon", "Repair the companion drone if it is available, or deploy a Scout Drone as fallback support. Either path gives you remote scouting before risky routes.")),
            Map.entry("upgrade_drone_support", guide("Drone Support", "Stable drone support means a repaired companion or an active Scout Drone. Use terminal drone controls or item commands before long expeditions.")),
            ...
            Map.entry("recover_drone_intel", guide("Drone Intel", "Drone intel represents a successful recon pass. Let the drone mark risk before you commit supplies and health.")),
            ...
            Map.entry("poi_explorer", guide("POI Exploration", "Points of interest are expedition loops: scan, prepare for the listed hazard, enter, recover the objective cache or log, then return before supplies collapse.")),
            ...
            Map.entry("build_nexus_capacitor", guide("Nexus Capacitor", "The Nexus Capacitor is late-grid storage. Use it behind high-voltage cable to absorb boss-route demand spikes and keep critical machines alive.")),
            ...
            Map.entry("find_nexus_core", guide("Nexus Core", "The Nexus Core is locked behind all nine buried guardian nodes. Stand near the unresolved Core after the Nexus Scar Avatar falls.")),
            ...
            Map.entry("reach_decision", guide("Nexus Decision", "The Nexus choice commits your endgame direction. Use the NEXUS tab near the unresolved Core, select a path once, then confirm it; the choice is permanent.")),
            Map.entry("restore_repair_nodes", guide("Restore Path", "Restoration repairs the grid instead of breaking it. Activate three Power Nodes; crafted replacements count.")),
            ...
            Map.entry("restore_epilogue", guide("Restore Epilogue", "The final Restore protocol resolves the repaired grid. Confirm the epilogue from MISSIONS after The Warden falls.")),
            ...
            Map.entry("destroy_epilogue", guide("Destroy Epilogue", "The final Destroy protocol ends grid authority. Confirm the epilogue from MISSIONS after The Warden falls.")),
            ...
            Map.entry("control_epilogue", guide("Control Epilogue", "The final Control protocol binds the wasteland network to your signal. Confirm the epilogue from MISSIONS after The Warden falls."))
    );
