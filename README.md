...
| `echoagriculturereclamation` | `0.1.0` | Field ecology recovery chapter for recovered seed capsules, contaminated soils, hydroponics, greenhouses, gene stabilization, and chunk-local restoration. |
...
6. Clear the eight active biome guardians by scanning surface entrances and descending into their buried Gridfall nodes.
...
- `echoagriculturereclamation` `0.1.0` - field agriculture and ecology recovery chapter.
...
- `addons/echoagriculturereclamation/build/libs/echoagriculturereclamation-0.1.0.jar`
...
Expected verification result: every required GameTest task reports clean completion across Core, Terminal, Ashfall, Orbital, Agriculture Reclamation, Stationfall, Nexus, Industrial, and Blackbox. If `checkEchoModJarSet` fails, run `.\gradlew.bat copyEchoJarsToModpack` first so the `Axes of Tomorrow` profile contains exactly one current jar for every ECHO module.
...
- **ECHO addon chapters:** Orbital Remnants, Agriculture Reclamation, Stationfall, Nexus Protocol, Industrial Nexus, and Blackbox Protocol are included in the public release stack and surface through ECHO Core plus ECHO Terminal.
